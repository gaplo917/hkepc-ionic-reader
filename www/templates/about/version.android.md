...
* ***「更多功能」彈出式界面以原生界面取代*** - 反應速度更快捷及更 Android 的界面
* ***加入「分享到...」功能*** - 用戶可以自由選擇「複制到剪貼薄」、「在 Chrome 打開」或「分享到其它社交平台例如 Telegram, WhatsApp」
* ***重新設計「主題列表」界面*** - 更便利、清楚和容易地使用子版塊
* ***重新設計「帖子頁面」界面*** - 更清楚顯示目前的標題以及所在的版塊
* ***重新設計「頁面控制器」*** - 統一位於下方，更方便單手轉頁
* ***重新設計「發送短消息對話」界面*** - 更改成一般對話 App 相似的界面令用戶更易上手、加入「黑夜」及「OLED 省電」設計及移除了 IR signature
* ***自動插入 Youtube 播放元件*** - 解決 EPC 用戶發佈主題時沒有使用 Youtube 插入功能，IR 用戶仍可以直接觀看短片，不用彈出彈入
* ***復審 ionic v1 內核*** - 調整內核代碼，大幅提升瀏灠速度；主要提升速度原因是移除了 IR App 不必要的 ~300ms 延遲（以往偶爾會 Freeze 的主因），亦加入了 `{passive: true}` 防止阻擋 ui-thread
* ***復審 ionic v1 內核*** - 移除 `ionic.keyboard` 相關代碼、修正 `ionic.tap` 問題；
* ***復審大量 v3.x 未協調的元件*** - 重製或更改元件，保持 IR App 一貫清新風格
* ***移除部份 v2.x 遺留的殘餘代碼*** - 提升開啟「主題/回覆發佈頁面」速度；主要是移除 Web 內核上載圖片的功能 (3.x 已一統改為用 App 內核壓縮兼上載)
* ***重構大量 v1.x 遺留的代碼*** - 將所有 HTML Parsing 的工作都交予 Web Worker 處理，保証 Main Thread 不會被 CPU-Intensive 工作阻擋 
* 改善英文字會斷開隔行問題
* 改善「論壇版塊」無法加載的訊息
* 改善「主題/回覆發佈頁面」標題太長時很難使用

#### App 更新
* ***大幅提升開 App 速度*** - 改善部份代碼，減低開 App 時至少 300% 負載率
* ***更省電*** - 改善部份代碼，會根據使用狀況暫停不必要的 WebView
* ***支援「黑夜」及「OLED省電模式」主題*** - 全部原生元件都支援
* ***自動開啟 YouTube App*** - 點擊任何 YouTube 都會自動開啟 YouTube App
...
3. 用戶方面：所有 HKEPC 用戶㫮可繼續完全免費享受 IR App。
...
作者有諗過直接關門大吉這個「三輸」局面
* 作者在 IR App 付出的努力將付諸流水
* HKEPC 用戶將不能再使用 IR App 輕鬆瀏覧 HKEPC
* HKEPC 將會失去每月 60 萬 PageView 及一堆已經主力用手機應用程式上的 HKEPC 用戶
以表達 「HKEPC 論壇於 2019 還沒有官方 App 」的不滿，但回顧初衷，作者對 HKEPC 有「熱衷」才能單方面付出三年時間做義工到今天，
相信無乜邊個可以做到呢個地步。

正因如此，作者亦不想 HKEPC 步向死亡，但「熱衷」終有一日會用盡，更何況 IR Pro v3 App「叫做暢順用到兼唔太多問題」，
剩餘的「熱衷」始終未夠大推動力去繼續進步。
...
由於 IR 定位特殊不能面向國際 (數十億人)，只能面向潛在 HKEPC 用戶(數千人)；若與其它面向國際的功能性 App 比較之下，潛在用戶人數簡直是天淵之別，故定價策略必需要有數據及算式支持。

要在僅有數千名的潛在用戶內，要從中獲得收入並達到收支平衝，定價無可否認一定要比較高。
...
只計算 Extreme v4 的巨型更新，所涉及費用大約 `25,000 HKD` (仍未計 v1/v2/v3 所付出的)

保守計算：估算會有 50% 日用戶、5% 週用戶及 0.5% 月用戶會購買 Extreme v4 支持
```
