...
;(function () {
              ...
              if (!foundInstancesCount && handle) {
                return $log.warn(
                  'Delegate for handle "' +
                    handle +
                    '" could not find a ' +
                    'corresponding element with delegate-handle="' +
                    handle +
                    '"! ' +
                    methodName +
                    '() was not called!\n' +
                    ...
                    methodName +
                    '() immediately, and ' +
                    'your element with delegate-handle="' +
                    handle +
                    '" is a child of your ' +
  ...
  ;(function (ionic) {
    ...
    ionic.EventController = {
      ...
      trigger: function (eventType, data, bubbles, cancelable) {
        ...
        ;(data && data.target && data.target.dispatchEvent && data.target.dispatchEvent(event)) ||
  ...
  ;(function (ionic) {
    ...
    ionic.Gestures.NO_MOUSEEVENTS =
      ionic.Gestures.HAS_TOUCHEVENTS && window.navigator.userAgent.match(ionic.Gestures.MOBILE_REGEX)
    ...
    ionic.Gestures.PointerEvent = {
      ...
      matchType: function (pointerType, ev) {
        ...
        var types = {}
        types[ionic.Gestures.POINTER_MOUSE] =
          ev.pointerType == ev.MSPOINTER_TYPE_MOUSE || ev.pointerType == ionic.Gestures.POINTER_MOUSE
        types[ionic.Gestures.POINTER_TOUCH] =
          ev.pointerType == ev.MSPOINTER_TYPE_TOUCH || ev.pointerType == ionic.Gestures.POINTER_TOUCH
        types[ionic.Gestures.POINTER_PEN] =
    ...
    ionic.Gestures.gestures.Drag = {
      ...
      handler: function dragGesture(ev, inst) {
        if (ev.srcEvent.type == 'touchstart' || ev.srcEvent.type == 'touchend') {
          ...
          if (
            inst.options.prevent_default_directions.length > 0 &&
        ...
        switch (ev.eventType) {
          ...
          case ionic.Gestures.EVENT_MOVE:
            ...
            // lock drag to axis?
            if (
              ionic.Gestures.detection.current.lastEvent.drag_locked_to_axis ||
            ...
            if (
              (inst.options.drag_block_vertical && ionic.Gestures.utils.isVertical(ev.direction)) ||
...
*/
  ...
  ionic.tap = {
    ...
    ignoreScrollStart: function (e) {
      return (
        e.defaultPrevented || // defaultPrevented has been assigned by another component handling the event
    ...
    requiresNativeClick: function (ele) {
      if (
        ...
        (ele.tagName == 'A' ||
          ele.tagName == 'BUTTON' ||
          ele.hasAttribute('ng-click') ||
          (ele.tagName == 'INPUT' && (ele.type == 'button' || ele.type == 'submit')))
      ) {
      ...
      if (
        ...
        ionic.tap.isLabelContainingFileInput(ele)
      ) {
        return true
      }
  ...
  function tapClickGateKeeper(e) {
    ...
    if (
      (ionic.scroll.isScrolling && ionic.tap.containsOrIsTextInput(e.target)) ||
  ...
  function tapMouseDown(e) {
    ...
    if (tapEnabledTouchEvents) {
      ...
      if (
        ...
        (!ionic.tap.isTextInput(e.target) || tapLastTouchTarget !== e.target) &&
        !isSelectOrOption(e.target.tagName) &&
        !e.target.isContentEditable &&
        !ionic.tap.isVideo(e.target)
      ) {
  ...
  function tapHasPointerMoved(endEvent) {
    ...
    var releaseTolerance =
      hasClassList && endEvent.target.classList.contains('button')
        ? TAP_RELEASE_BUTTON_TOLERANCE
        : TAP_RELEASE_TOLERANCE

    return (
      Math.abs(tapPointerStart.x - endCoordinates.x) > releaseTolerance ||
  ...
  ionic.Platform.ready(function () {
    // gap removed all, should here do something?
  })
  ...
  ;(function (ionic) {
    ...
    ionic.views.Scroll = ionic.views.View.inherit({
      initialize: function (options) {
        ...
        self.hintResize = ionic.debounce(
          function () {
            self.resize()
          },
          1000,
  ...
  */
      ...
      __initEventHandlers: function () {
        ...
        container.addEventListener('scrollChildIntoView', self.scrollChildIntoView, { passive: true })
        ...
        document.addEventListener('resetScrollView', self.resetScrollView, { passive: true })
        ...
        if (self.__indicatorY) {
          ...
          if (self.__scrollTop < 0) {
            heightScale = Math.max(
              self.__indicatorY.minScale,
              (self.__indicatorY.size - Math.abs(self.__scrollTop)) / self.__indicatorY.size
            )
            ...
            heightScale = Math.max(
              self.__indicatorY.minScale,
              (self.__indicatorY.size - heightDiff) / self.__indicatorY.size
            )
          ...
          if (self.__isSingleTouch && self.options.animating && timeStamp - self.__lastTouchMove <= 100) {
            ...
            if (startPos !== endPos) {
              ...
              var minVelocityToStartDeceleration =
                self.options.paging || self.options.snapping
                  ? self.options.decelVelocityThresholdPaging
                  : self.options.decelVelocityThreshold
  ...
  */
      ...
      __publish: function (left, top, zoom, animate, wasResize) {
        ...
        if (animate && self.options.animating) {
          ...
          self.__isAnimating = zyngaCore.effect.Animate.start(
            step,
            verify,
            completed,
  ...
  */
      ...
      __startDeceleration: function () {
        ...
        var verify = function () {
          var shouldContinue =
            Math.abs(self.__decelerationVelocityX) >= self.__minVelocityToKeepDecelerating ||
  ...
  ;(function (ionic) {
     ...
     */
      ...
      __initEventHandlers: function () {
              ...
              ionic.requestAnimationFrame(function () {
                ...
                scrollViewOffsetHeight = Math.max(
                  ...
                  Math.min(
                    ...
                    self.__originalContainerHeight - (e.detail.keyboardHeight - 43)
                  )
        ...
        container.addEventListener('scroll', self.onScroll, { passive: true })

        //Broadcasted when keyboard is shown on some platforms.
        //See js/utils/keyboard.js
        container.addEventListener('scrollChildIntoView', self.scrollChildIntoView, { passive: true })

        container.addEventListener(ionic.EVENTS.touchstart, self.handleTouchMove, { passive: true })
        container.addEventListener(ionic.EVENTS.touchmove, self.handleTouchMove, { passive: true })
  ...
  ;(function (ionic) {
    ...
    SlideDrag.prototype.drag = ionic.animationFrameThrottle(function (e) {
      ...
      if (
        !this._isDragging &&
        (Math.abs(e.gesture.deltaX) > this.dragThresholdX || Math.abs(this._currentDrag.startOffsetX) > 0)
      ) {
    ...
    ReorderDrag.prototype._getReorderIndex = function () {
      ...
      var siblings = Array.prototype.slice
        .call(self._currentDrag.placeholder.parentNode.children)
        .filter(function (el) {
      ...
      for (var i = 0, len = siblings.length; i < len; i++) {
        ...
        if (i === len - 1) {
          ...
          dragOffsetTop > el.offsetTop - el.offsetHeight / 2 &&
    ...
    ionic.views.ListView = ionic.views.View.inherit({
      ...
      _startDrag: function (e) {
        ...
        if (
          ionic.DomUtil.getParentOrSelfWithClass(e.target, ITEM_REORDER_BTN_CLASS) &&
          (e.gesture.direction == 'up' || e.gesture.direction == 'down')
        ) {
