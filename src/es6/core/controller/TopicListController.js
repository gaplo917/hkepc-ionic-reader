...
export class TopicListController {
  ...
  constructor($scope, LocalStorageService, AuthService, ngToast, apiService, rx, observeOnScope, $q, $state) {
    ...
    $scope.$on('$ionicView.enter', (e) => {
      ...
      this.authService
        .getUsername()
        .safeApply(this.scope, (username) => {
          this.username = username
        })
        .subscribe()

      this.localStorageService
        .get('topics-cache-timestamp')
  ...
  loadList() {
    ...
    this.topicsSubscription = this.apiService
      .topicList()
  ...
  canShowIniOSReview(topicId) {
    ...
    return (
      ...
      blackList.indexOf(parseInt(topicId)) < 0 ||
      (this.isLoggedIn && this.username !== 'logary917')
    )
  }
