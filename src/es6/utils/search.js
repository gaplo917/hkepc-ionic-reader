...
const searchAndInjectHighlightBetweenKeyword = (content, keyword) => {
  ...
  const validIndexes = searchKeywordIndex(content.toLowerCase(), keyword.toLowerCase())
    .filter(index => !isIndexInBrace(content, bracePos, index)) // we do not want accidentally search the html tag
...
export const searchMultipleKeyword = (keywordArr, contentToSearch, result = { matches: 0 }) => {
  switch (keywordArr.length) {
    ...
    default :
      const title = angular.element('<div/>').html(result.hlContent || contentToSearch).html()
